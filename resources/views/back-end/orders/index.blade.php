                                    ...
                                    @endif
                                     ...
                                     <td>
                                        <form action="{{ route($routeName.'.destroy' , ['id' => $item]) }}" method="post">
                                            ...
                                            {{ method_field('delete') }}
                                            @if($status < 4)
                                            <a href="{{ url('admin/change-status-order/'. ($status + 1).'/'.$item->id) }}" rel="tooltip" title="" class="btn btn-info" data-original-title="Edit {{ $sModuleName }}">
                                                <i class="material-icons">تم</i>
                                            </a>
                                            @elseif(isset($item->delivery))
                                            <a href="{{ url('admin/change-status-order/'. ($status + 1).'/'.$item->id.'/'.$item->delivery->id) }}" rel="tooltip" title="" class="btn btn-info" data-original-title="Edit {{ $sModuleName }}">
                                                <i class="material-icons">تم</i>
                                            </a>
